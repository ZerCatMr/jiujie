...
layui.define(['view',], function (exports) {
  ...
  element.on('tabDelete(layadmin-layout-tabs)', function (obj) {
    ...
    //当tabspage删除时，框架会自动切换到现有tab上
    //模拟tab点击
    if(obj.index && obj.index>1){ //如果只剩首页 则首页刷新
      admin.tabsPage.type = 'tab';
    }
    setThisRouter(othis);
    ...
    // //监听 角色切换的
   $body.on('click','*[lay-role]',function(){
     debugger
      var value=$(this).attr('value');
      hdk.ajax({
        api:"/api/login/cutRole",
        data: JSON.stringify({"token":sessionStorage.getItem("health-login-cache-token"),"cutroleid":(JSON.parse(sessionStorage.getItem("health-login-cache")).roleDOList[value].roleid)})
      },function (result) {
        //数据存入缓存
