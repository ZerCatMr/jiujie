layui.define(function (exports) {
    ...
    layui.use(['laydate', 'table', 'element', 'hdk', 'form', 'tool', 'appconfig'], function () {
        ...
        $('.health-query .multi-checkbox div', hdk.config.activeTab).on('click', function () {
            ...
            let checkStr = "";
            for (let i = 0; i < $("#examwork_fillinperson_edit .hyd-selected").length; i++) {
                checkStr += $("#examwork_fillinperson_edit .hyd-selected").eq(i).find("span").html() + " ";
            }
            loadUserByReport(tool.filterArray(checkStr, testData));
        });

        //监听筛选框
        $('#examwork_fillinperson_edit input[name="health-search"]', hdk.config.activeTab).keyup(function () {
            //重载方法
        ...
        function loadUserByReport(data){
            //先清除之前的所有数据
            $("#examwork_fillinperson_edit #center_card_main").empty();

            //执行一个laypage实例
            laypage.render({
                elem: 'fillin_person_edit_page' //注意，这里的 test1 是 ID，不用加 # 号
                ,count: data.length //数据总数，从服务端得到
                ,limit:16//每页显示数量
                ,jump: function (obj, first) {
                    let thisPage = obj.curr;
                    //使得当前页显示,其他页码隐藏
                    $('#examwork_fillinperson_edit #center_card_main [thisPage="'+thisPage+'"]').css("display", "block");
                    $('#examwork_fillinperson_edit #center_card_main .userDiv:not([thisPage="'+thisPage+'"])').css("display", "none");
                }
            });
            let totalPageNum = (data.length  - 1) / 16 + 1;//总页数 = (总条数-1)/每页显示条数+1;

            for(let i=0; i<data.length; i++){
                //计算当前页码  当前页码数 = (当前条数/每页显示条数)向上取整
                var page= Math.ceil((i+1)/14);
                const thisDO = data[i];
                var borderName = null;
                ...
                var userDiv =
                    ...
                    '</div>' +
                    '</div>';
                $('#examwork_fillinperson_edit #center_card_main' ,hdk.config.activeTab).append(userDiv);
                //默认只显示第一页的数据
                $('#examwork_fillinperson_edit #center_card_main [thisPage="1"]').css("display", "block");
                $('#examwork_fillinperson_edit #center_card_main .userDiv:not([thisPage="1"])').css("display", "none");
            }
        }
        ...
        $("#import_user").click(function () {
            ...
            layer.open({
                ...
                btn: ['确认','关闭'],
                area: ['700px', '550px'],
                content: $("#examworkfillinpersonedit_addperson_layer",hdk.config.activeTab).html(),
                success: function () {
                    //加载表单
                    form.render();
                }
        ...
        //点击减少用户
        $("#examwork_fillinperson_edit .remove_user").click(function () {
            //打开减少人员弹窗
            layer.open({
                type: 1,
                title: '减少人员信息',
                ...
                area: ['700px', '550px'],
                btnAlign: 'c',
                content: $("#examworkfillinpersonedit_dellperson_layer", hdk.config.activeTab).html(),
                success: function () {
                }
            });
        });
        //点击编辑人员详情
        $('#examwork_fillinperson_edit .edit_user').click(function () {
            //打开编辑用户弹窗
            layer.open({
                type: 1,
                title: '编辑人员信息',
                btn: ['确认','关闭'],
                area: ['800px', '600px'],
                content: $("#examworkfillinpersonedit_editperson_layer", hdk.config.activeTab).html(),
                btnAlign: 'c',
        ...
        //点击保存按钮
        $("#examwork_fillinperson_edit #save").click(function () {
            //弹出询问框
            //打开确认管理员信息弹窗
            layer.open({
                type: 1,
                ...
                btn: ['确认','关闭'],
                area: ['700px', '550px'],
                content: $("#examworkfillinpersonedit_confirmmanage_layer",hdk.config.activeTab).html(),
                success: function () {
                    //加载表单
                    form.render();
                }
        ...
        //点击取消按钮
        $("#examwork_fillinperson_edit #cancel").click(function () {
            //关闭当前页面
            admin.closeThisTabs();
        });
        //点击编辑管理人员信息
        $("#examwork_fillinperson_edit #top_card [name=\"left/right\"] a").click(function () {
            layer.open({
                type: 1,
                title: '编辑人员信息',
                btn: ['确认','关闭'],
                area: ['700px', '550px'],
                content: $("#examworkfillinpersoneidt_editmanage_layer",hdk.config.activeTab).html()
            });
        });

    });
    exports('examwork/fillin_person_edit', {})
});
